...
module.exports = function(Polyglot, polyInterface) {
  ...
  class RingInterface {
        ...
        .then(function(oauth) {
          ...
          if (_this.oauthIsValid(oauth)) {
            ...
            if (controller) {
              ...
              logger.error('Controller node not found; cannot proceed with device discovery. Please try a discovery later in the admin console using the controller node.');
            }

          } else {
            logger.error('Invalid oAuth result %o', oauth);
          }
        });
      }
    }
    ...
    async getOwnerDevices() {
      ...
      try {
        if (devices && !(polyInterface.getStage() === 'test' && acceptSharedDevices)) {
    ...
    eventProcessor(req, res) {
      ...
      if (pragma === this.pragma) {
        ...
        if (controls.includes(eventData.event)) {
          ...
          logger.error('Unhandled event received %o', eventData);
        }
