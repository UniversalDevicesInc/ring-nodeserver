...
const ringApiBasePath = '/integrations_api';
  ...
  class RingInterface {
    ...
    sendAuthNoticeMessage() {
      ...
      if (!notices[authNoticeKey]) {
        // logger.info('oaParams AUTH: %o', oaParams);
    ...
    async processAuthCode(code, state, controllerAddress) {
      ...
      if (state !== config.worker) {
        ...
        // logger.info('oaParams TOKEN: %o', oaParams);
    ...
    async subscribe() {
      ...
      if (nodesCount >= 2) {
        if (netInfo && netInfo.publicIp && netInfo.publicPort) {
          const url = 'http://' + netInfo.publicIp + ':' + netInfo.publicPort +
            '/event';
          ...
          try {
            ...
            const subResult = await this.callApiPatch('/subscription', {
              subscription: {
                postback_url: url,
                metadata: {
                  headers: {
          ...
          logger.error('Cannot subscribe. Incorrect netInfo: %o', netInfo);
        }
      } else {
    ...
    eventProcessor(req, res) {
      ...
      if (pragma === this.pragma) {
        // Valid event?
        if (Object.keys(control).includes(eventData.event)) {
          ...
          if (node) {
            logger.info('Event received %s for %s %s',
              eventData.event, id, node.description);

            node.reportCmd(control[eventData.event]);
          } else {
