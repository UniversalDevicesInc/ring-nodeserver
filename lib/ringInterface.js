...
module.exports = function(Polyglot, polyInterface) {
  ...
  class RingInterface {
    ...
    // Works for Doorbells or Cams
    async getDeviceData(id, preFetchedDevices = null) {
      const devices = preFetchedDevices || await this.getDevices();
      const all = devices.doorbells
      .concat(devices.authorized_doorbells)
    ...
    async subscribe() {
      ...
      //     publicPort: 3000,
      //   };
      //   protocol = 'http'; // We talk directly to the nodeserver in dev
      // }

      if (nodesCount >= 2) {
        if (netInfo && netInfo.publicIp && netInfo.publicPort) {
          const url = protocol + '://' + netInfo.publicIp + ':'
            + netInfo.publicPort + '/event';
